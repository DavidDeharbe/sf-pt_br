...
(** É claro que nem tudo pode ser provado por simples cálculos:
    geralmente, valores hipotéticos desconhecidos (números 
    arbitrários, booleanos, listas, etc.) podem bloquear o cálculo. 
    Por exemplo, se nós tentarmos provar o seguinte fato usando a 
    tática do [simpl] (_simplificação_) como acima, nós ficamos 
    presos. *)
...
(** Use as táticas que você aprendeu até agora para provar o seguinte
    teorema a respeito de funções booleanas. *)
...
(** Agora crie e prove um teorema [negation_fn_applied_twice] similar
    ao anterior, mas que na segunda hipótese diga que a função [f] 
    tem a propriedade [f x = negb x]. *)
...
(** Para cada tipo de símbolo de notação no Coq, podemos especificar
 ...
 0 a 100 e associatividade _esquerda_ ou _direita_ ou _nenhuma_ associatividade.
    ...
    O escopo da notação também se aplica à notação numeral (3,4,5, etc.), logo
    você pode às vezes ver [0%nat], o que significa [0], ou [0%Z] o que significa 
    o  numero inteiro 0. *)
